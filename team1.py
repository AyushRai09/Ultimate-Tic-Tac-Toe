...
class Player1:
		...
		self.uptoMaxDepth=5
		self.num=0
        def utilityOfState(board, old_move, currentMarker):
		return 0;
	
        def minimax(board,old_move,currentMarker,depth, alpha,beta, parentAlpha, parentBeta, bestRow, bestCol):
				...
				for j in range(4*col, 4*col+4):
					if(board[i][j]=='-'):
						...
						if(alpha<beta):  #only call if this condition exists, otherwise prune it. That's why in the else condition, "break" is used.
							utility=self.minimax(board,new_move,nextMarker,depth+1, -100000.0, 100000.0, alpha, beta, bestRow, bestCol)
		...
		utility, bestRow,bestCol= minimax(temp_board, old_move, currentMarker, 0, -100000.0, 100000.0, -100000.0, 100000.0,-1,-1)
